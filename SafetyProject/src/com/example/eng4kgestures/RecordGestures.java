...
public class RecordGestures extends Activity implements SensorEventListener {
	...
	String INSTRUCTIONS = "To record a gesture, enter a valid gesture name. Then press and hold the volume down/up key and perform the gesture that you want to record.";
	...
	protected void processData(double d) {
		...
		if (!accelerationList.isEmpty()) {
			...
			recordingStatus.setText("Gesture saved, initial size was " + size
	...
	public void onClickStartRecording(View v) {
		// Show the dialog with instructions on how to record a gesture
		showRecordGestureInstructions();
	}

	public void onClickDeleteGestures(View v) {
		Intent deleteGesture = new Intent(this, DeleteGestures.class);
		startActivity(deleteGesture);
	}

	public boolean validate() {
		if (!gestureName.getText().toString().isEmpty())
			return true;
		else
			gestureName.setError("Please enter a valid gesture name!");
		return false;
	}

	private void showRecordGestureInstructions() {
		AlertDialog notificationDialog = new AlertDialog.Builder(this)
				.setTitle("Record Gesture").setMessage(INSTRUCTIONS)
